...
%   $Revision: 1.1.6.5 $  $Date: 2011/05/13 18:08:07 $
...
if exp(logCB) < realmin
    ...
    switch Form
        case 2
            ...
            while outerArg*(1-innerSum)>realmin
                m = m + 1;
                k = k + 1;
            ...
            m = 0;
            delta = expA(Y,m);
            while delta > realmin && m<=N-1
                outerSum = outerSum + delta;
                m = m + 1;
                delta = expA(Y,m);
            end
...
x = 1/(12*z+2/(5*z+53/(42*z+1170/(53*z+53/z))));   % Eq. B3
%--------------------------------------------------------------------------
%-----
